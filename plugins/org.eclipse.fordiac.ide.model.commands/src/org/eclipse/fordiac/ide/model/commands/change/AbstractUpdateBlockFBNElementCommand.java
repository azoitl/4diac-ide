...
public abstract class AbstractUpdateBlockFBNElementCommand extends Command
	...
	private void handleErrorMarker() {
		if ((oldElement instanceof final ErrorMarkerFBNElement oldErrorMarkerEl)
				&& (newElement instanceof final ErrorMarkerFBNElement newErrorMarkerEl)) {
			final FBNetworkElement repairedElement = oldErrorMarkerEl.getRepairedElement();
			if (repairedElement != null) {
				newErrorMarkerEl.setRepairedElement(repairedElement);
			}
		}
	}
	...
	private static boolean hasNonInternalAttr(final IInterfaceElement ie) {
		return !ie.getAttributes().stream().filter(attr -> !InternalAttributeDeclarations.isInternalAttribute(attr))
				.toList().isEmpty();
	}
